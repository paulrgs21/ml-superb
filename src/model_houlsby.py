...
class HoulsbyAdapter(nn.Module):
    """
    Simple Houlsby-style bottleneck adapter:
    x + W_up(dropout(act(W_down(x))))
    """
...
def _patch_hubert_layer_with_houlsby(layer, hidden_size, bottleneck=32, adapter_dropout=0.1):
    """
    Patch one HubertEncoderLayer to insert:
    - one adapter after self-attention residual block
    - one adapter after feed-forward residual block
    """
    ...
    def houlsby_forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        # Houlsby adapter after attention block
        ...
        # Houlsby adapter after FFN block
...
class HuBERT_CTC_Houlsby(nn.Module):
    """
    Same ML-SUPERB architecture as baseline / LoRA,
    but with Houlsby adapters inserted in HuBERT encoder layers.
    """
    def __init__(
        self,
        vocab_size,
        ...
        # 2. Patch each Transformer encoder layer with Houlsby adapters
        for layer in self.encoder.encoder.layers:
            _patch_hubert_layer_with_houlsby(
                layer,
                hidden_size=hidden_size,
